...
var Assessment = (function () {
    ...
    Assessment.prototype.validate = function () {
        ...
        var QUESTION_MAX_LENGTH = 65, ANSWER_MAX_LENGTH = 65;
        // loop through all questions
        self.questions.forEach(function (element, index) {
            ...
            element.answers.forEach(function (element) {
                if (!((element.substr(0, 2) == '* ') || (element.substr(0, 2) == '- ')))
     ...
     */
    Assessment.prototype.createAssessmentFile = function (courseId, moduleNumber, buildPath) {
        var self = this;
     ...
     */
    Assessment.getQuestionFromXmpMarker = function (adobeXmpMarkerString) {
     ...
     */
    Assessment.loadQuestionsFromXmpFiles = function (xmpFiles) {
        var deferred = Q.defer(), xmpMarker = new XmpMarker();
        var assessment = new Assessment();
        var promises = [];
        // for each XMP file, get all the markers...
        xmpFiles.forEach(function (xmpFile) {
            // then get all the markers for each one
            promises.push(xmpMarker.getMarkers(xmpFile).then(function (markers) {
     ...
     * @returns {Promise<Assessment>}             Assessment object with questions.
     */
    Assessment.loadQuestionsFromMetaFile = function (questionFilePath) {
        var deferred = Q.defer();
        var assessment = new Assessment();
        var question;
        ...
        liner.on('line', function (line) {
            ...
            if (line.substr(0, 2) == '= ') {
                var clipFilenameParts = line.substr(2, line.length).split('.')[0].split('-');
